...
async def main_async(args):
    ...
    brand_slug = getattr(args, "brand_slug", "stelz")
    brand_row = sb.table("brands").select("id, slug").eq("slug", brand_slug).execute().data
    if not brand_row:
        sys.exit(f"No brand found with slug={brand_slug}")
    ...
    try:
        ...
        # Selection modes:
        #   default               — adaptive: only creators where next_scan_at <= now()
        #                           (set per-creator by update_creator_cadence based on
        #                           last 28d posting rate)
        #   --prioritize-hits     — known hit-producers first; ignores adaptive gate
        #   --no-adaptive         — fall back to the old `last_scraped_at NULLS FIRST` rotation
        cols = "id, handle, last_scraped_at, hits_seen, cadence_class, next_scan_at"
        if args.prioritize_hits:
            ...
            creators_q = (sb.table("creators")
                          ...
                          .eq("platform", "instagram")
                          .in_("status", ["discovered", "promoted"])
                          .or_(f"next_scan_at.is.null,next_scan_at.lte.{now_iso_q}")
                          .order("next_scan_at", nullsfirst=True))
        if args.max_creators:
            creators_q = creators_q.limit(args.max_creators)
        creators = creators_q.execute().data or []
        # Hard floor: never re-scrape a creator scraped <skip_recent_hours ago.
        # Acts as a safety net when adaptive scheduling is too aggressive AND
        # is the primary gating mechanism for --prioritize-hits / --no-adaptive.
        ...
        # Build a map of creator_id -> cadence_class so we can write the next
        # adaptive scan window per creator. Use the value we fetched earlier;
        # creators without a cadence yet fall back to 'mid' (12h gap).
        cadence_map = {c["id"]: (c.get("cadence_class") or "mid") for c in creators}
        gap_by_class = {
            "active": timedelta(hours=4),
            "mid": timedelta(hours=12),
            "dormant": timedelta(hours=48),
        }
        if creator_updates:
            t_cu = time.time()
            rows = []
            for cid, v in creator_updates.items():
                gap = gap_by_class.get(cadence_map.get(cid, "mid"), timedelta(hours=12))
                rows.append({
                    "id": cid,
                    "brand_id": brand_id,
                    ...
                    **v,                       # provides "handle" + "follower_count"
                    ...
                    "next_scan_at": (now + gap).isoformat(),
                })
        ...
        # Strip the carrier key + dedupe by (platform, external_id) before upserting.
        # Same post can be returned for multiple creators (cross-posts, tagged posts),
        # which causes "ON CONFLICT DO UPDATE command cannot affect row a second time".
        seen_keys = set()
        ci_insert = []
        for r in ci_rows:
            key = (r.get("platform"), r.get("external_id"))
            if key in seen_keys:
                continue
            seen_keys.add(key)
            ci_insert.append({k: v for k, v in r.items() if k != "_display_url"})
        ...
        if ci_insert:
            ...
            print(f"  content_items upsert: {time.time()-t_ci:.1f}s for {len(ci_insert)} rows ({len(ci_rows)-len(ci_insert)} dupes skipped)", file=sys.stderr)
...
def main():
    ...
    parser.add_argument("--brand-slug", dest="brand_slug", default="stelz",
                        help="Brand to scan (multi-tenant). Defaults to stelz.")
    parser.add_argument("--prioritize-hits", action="store_true",
                        help="Order creators by hits_seen DESC (top hit-producers first). "
                             "Combine with --max-creators to do quick priority sweeps.")
    parser.add_argument("--no-adaptive", action="store_true",
                        help="Disable adaptive scheduling (next_scan_at gate). "
                             "Falls back to old last_scraped_at NULLS FIRST rotation.")
