...
async def main_async(args):
    ...
    # SRS GATE: Only Gemini-score creators whose SRS is high enough to be worth it.
    # Cuts API calls by ~80% in production. Bypass with --no-srs-gate or --srs-gate 0.
    if args.srs_gate > 0 and creators:
        # Pull SRS for the brand
        srs_rows = (sb.table("resonance_scores")
                    .select("creator_handle, platform, srs")
                    .eq("brand_id", brand_id)
                    .gte("srs", args.srs_gate)
                    .execute()).data or []
        gated_keys = {(r["creator_handle"].lower(), r["platform"]) for r in srs_rows}
        before = len(creators)
        creators = [c for c in creators
                    if (c.get("handle","").lower(), c.get("platform","")) in gated_keys]
        print(f"SRS gate (>={args.srs_gate}): {before} -> {len(creators)} creators "
              f"({before - len(creators)} skipped, ~{round(100*(before-len(creators))/max(before,1))}% saved)",
              file=sys.stderr)
...
def main():
    ...
    parser.add_argument("--srs-gate", type=float, default=60.0,
                        help="Only score creators with SRS >= this. 0 disables gating. Default 60.")
